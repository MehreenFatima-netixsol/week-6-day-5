...
# CORS: required for any browser-based client (ui_chat.html opened as a local file,
# Streamlit running on its own port, or any frontend served from a different
# origin/port than this API). Without this, the browser's preflight OPTIONS request
# on POST /chat is rejected with 405 and the actual request never gets sent -- the UI
# looks like it can't reach the API even though `GET /health` (a "simple" request, no
# preflight) works fine. Wide open (`allow_origins=["*"]`) is appropriate for this demo
# deployment; lock this down to specific origins before any real production exposure.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
